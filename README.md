# OpenProject Plugin Template

This is a template repository for creating new OpenProject plugins. It includes example code demonstrating common plugin patterns, and a setup script to quickly transform this template into your own plugin.
...
![Use this template button](images/use-this-template-button.png?raw=true)

### 2. Clone and Setup

```bash
# Clone your new repository
git clone git@github.com:your-org/openproject-my_feature.git
cd openproject-my_feature

# Run the setup script with your plugin name (snake_case)
./scripts/setup-plugin.sh my_feature

# Or use --clean to remove all example code
./scripts/setup-plugin.sh my_feature --clean
```

The setup script will:
- Rename all files and directories from `custom_links` to your plugin name
- Update all module references (`CustomLinks` -> `MyFeature`)
- Create SonarQube configuration (`.github/workflows/sonarqube.yml`)
- Update the gemspec with placeholder author info
- Optionally remove all example code (`--clean` flag)
...
```bash
git add .
git commit -m "Initialize plugin: openproject-my_feature"
git push
```

## Pre-requisites

In order to develop your plugin, you will need a working OpenProject core development environment:
...
We assume familiarity with Ruby on Rails, controllers, views, asset management, hooks and engines.

For more information, see the [OpenProject plugin development guide](https://www.openproject.org/docs/development/create-openproject-plugin).

## Integrating with OpenProject

Add your plugin to OpenProject by creating a `Gemfile.plugins` in the OpenProject core directory:

```ruby
group :opf_plugins do
  gem "openproject-my_feature", path: "/path/to/openproject-my_feature"
end
```
...
```bash
./bin/setup_dev
bundle exec rails db:migrate  # if your plugin has migrations
```
...
Verify installation at http://localhost:3000/admin/plugins

---

## Plugin Development Guide

The sections below explain common plugin features using examples in this template. Each section lists relevant files and explains the patterns.
